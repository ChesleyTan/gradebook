...
def insert(email, password, name):
    ...
    if (not exists(email)):
        ...
        return (False, "Registration error: A user with that email already exists.")

def validate_registration(email, password):
    if (len(email) < 1):
        return (False, "Registration error: Your email must be at least 1 character long")
    if (len(password) < 6):
...
def update(email, new_email=None, name=None, password=None, classes=None):
    if(exists(email)):
        updateDict = {}
